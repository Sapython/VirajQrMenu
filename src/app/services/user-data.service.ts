...
export class UserDataService {
  ...
  userDoc:any;
  constructor(private firestore: Firestore,private dataProvider: DataProvider,private router: Router,private alertify:AlertsAndNotificationsService) {
  ...
  public async setGoogleUserData(user:User,userData:ExtraLoginGoogleInfo){
    ...
    await setDoc(this.userDoc,data).then(()=>{
      this.alertify.presentToast('Welcome to Dosa Plaza')
    });
