...
import { Firestore, collectionData, collection, DocumentReference, doc, docData, DocumentData, getDoc } from '@angular/fire/firestore';
import { Auth,
  authState,
  signInAnonymously,
  signOut,
...
export class AuthenticationService {
  ...
  public async loginEmailPassword(email: string, password: string){
    ...
    let data = await signInWithEmailAndPassword(this.auth, email, password).then((credentials:UserCredential)=>{
      this.router.navigate(['/']);
      this.alertify.presentToast('Logged In.','info',5000);
      ...
      user.subscribe(u => {
        if (u) {
          ...
          this.userServerSubscription = docData(this.userDoc).subscribe((data:any) => {
            ...
            // console.log("User data from firestore",this.dataProvider.userData)
            this.dataProvider.gettingUserData= false;
            // console.log("User data from data provider",this.dataProvider.gettingUserData)
            //this.user = data;
          })
        }
      });
